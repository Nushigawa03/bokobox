 ...
 * @file BokoboxSrc.h
 */
#ifndef BOKOBOX_SRC_H_
#define BOKOBOX_SRC_H_
...
public:
    enum ParamId {  // MAGIC CHAR = 'Y'
         ...
         * BokoboxSrc::SCALE_CHROMATIC を設定すると12音、
         * BokoboxSrc::SCALE_C_MAJOR を設定すると7音(Cメジャースケール)が演奏できます。
         */
        PARAMID_SCALE,
        /**
    ...
    /**
     * @brief @~japanese BokoboxSrc オブジェクトを生成します。
     * @param[in] filter @~japanese 後ろにつなげる Filter オブジェクトを指定します。
     */
    BokoboxSrc(Filter& filter);
    BokoboxSrc(uint8_t perform_pin, uint8_t volume_pin, Filter& filter);
