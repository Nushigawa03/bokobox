 ...
 * SoundCaptureを継承したクラスは SoundCapture::onCapture() 関数をオーバーライドすることで、簡単に音声パラメータを扱うことができます。
 * SoundCaptureを使用するためには、SpresenseのSubCore1にYuruHorn_SubCore1.ino(音声を解析するスケッチ)をあらかじめ書き込んでおく必要があります。
 * @code {.cpp}
 * #include <SoundCapture.h>
 *
 * class Buzzer : public SoundCapture {
 * public:
 *     Buzzer(Filter& filter): SoundCapture(filter), prev_vol_(0) {
 *     }
 *
 ...
 *             SoundCapture::sendNoteOn(60, DEFAULT_VELOCITY, DEFAULT_CHANNEL);
 *         }
 *         if (prev_vol_ >= 400 && volume < 400) {
 *             // stop voice => stop sound
 *             SoundCapture::sendNoteOff(60, DEFAULT_VELOCITY, DEFAULT_CHANNEL);
 *         }
 *         prev_vol_ = volume;
 *     }
 *
 ...
 */
class SoundCapture : public BaseFilter {
#else
class SoundCapture {
#endif  // ARDUINO_ARCH_SPRESENSE
public:
    ...
    /**
     * @brief @~japanese SoundCapture オブジェクトを生成します。
     * @param[in] filter @~japanese 後ろにつなげる Filter オブジェクトを指定します。
     */
    SoundCapture(Filter& filter);
...
protected:
     ...
     * SoundCaptureを継承したクラスは SoundCapture::onCapture() 関数をオーバーライドしてください。
     * @param[in] freq_numer @~japanese 周波数の分子
     * @param[in] freq_denom @~japanese 周波数の分母
     * @param[in] volume @~japanese 音量
     */
    virtual void onCapture(unsigned int freq_numer, unsigned int freq_denom, unsigned int volume, unsigned int volume2) = 0;
