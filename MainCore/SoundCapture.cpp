...
static const char kClassName[] = "SoundCapture";
...
static SoundCapture* g_sound_capture = nullptr;
...
static bool micFrontendCallback(AsMicFrontendEvent evtype, uint32_t result, uint32_t sub_result) {
    if (g_sound_capture) {
        return g_sound_capture->onMicFrontend(evtype, result, sub_result);
    }
    return true;
}
#endif
...
static void frontendDoneCallback(AsPcmDataParam param) {
    if (g_sound_capture) {
        g_sound_capture->onFrontendDone(param);
    }
}

SoundCapture::Recorder::Recorder()
...
bool SoundCapture::Recorder::start() {
...
bool SoundCapture::Recorder::end() {
    if (state_ == RECORDER_STATE_INIT) {
        return true;
    }
...
void SoundCapture::Recorder::writeFrame(void* buffer) {
    if (state_ == RECORDER_STATE_RECORDING) {
...
void SoundCapture::Recorder::flush() {
...
bool SoundCapture::Recorder::isRecording() {
    return (state_ != RECORDER_STATE_INIT);
}

int SoundCapture::Recorder::getRecordingTime() {
    return recording_time_;
}

bool SoundCapture::Recorder::setRecordingTime(int value) {
    if (0 < value) {
...
SoundCapture::SoundCapture(Filter& filter)
    : BaseFilter(filter),
      gain_(kDefaultGain),
...
SoundCapture::~SoundCapture() {
    setParam(SoundCapture::PARAMID_RECORDING, false);
    if (g_sound_capture == this) {
        g_sound_capture = nullptr;
    }
}

bool SoundCapture::begin() {
    bool ok = true;
    int ret = 0;
    err_t err = FRONTEND_ECODE_OK;

    g_sound_capture = this;
    ...
    err = g_frontend->init(AS_CHANNEL_STEREO, AS_BITLENGTH_16, MIC_FRAME_LENGTH, AsDataPathCallback, callback);
    if (err != FRONTEND_ECODE_OK) {
    ...
    frontend_init.channel_number = AS_CHANNEL_STEREO;
    ...
    debug_printf("[%s::%s] SoundCapture: ready\n", kClassName, __func__);
...
void SoundCapture::update() {
    ...
    SoundCapture::Result analyze_result = {0, 0, 0, 0, 0, 0, 0};

    recorder_.flush();

    int8_t rcvid = -1;
    SoundCapture::Result* result = nullptr;
    ...
    if (has_result) {
        ...
        onCapture(analyze_result.freq_numer, analyze_result.freq_denom, analyze_result.volume, analyze_result.volume2);
    }
    BaseFilter::update();
}

bool SoundCapture::isAvailable(int param_id) {
    if (param_id == SoundCapture::PARAMID_MIC_GAIN) {
        return true;
    } else if (param_id == SoundCapture::PARAMID_INPUT_LEVEL) {
        return true;
    } else if (param_id == SoundCapture::PARAMID_RECORDING_TIME) {
        return true;
    } else if (param_id == SoundCapture::PARAMID_RECORDING) {
        return true;
    }
    return BaseFilter::isAvailable(param_id);
}

intptr_t SoundCapture::getParam(int param_id) {
    if (param_id == SoundCapture::PARAMID_MIC_GAIN) {
        return gain_;
    } else if (param_id == SoundCapture::PARAMID_INPUT_LEVEL) {
        return input_level_;
...
bool SoundCapture::setParam(int param_id, intptr_t value) {
    if (param_id == SoundCapture::PARAMID_MIC_GAIN) {
        int prev = gain_;
...
#if 0
bool RecodeSoundCapture::onMicFrontend(AsMicFrontendEvent evtype, uint32_t result, uint32_t sub_result) {
    return true;
}
#endif

void SoundCapture::onFrontendDone(AsPcmDataParam param) {
    static uint8_t buffer[CAP_BLOCK_SIZE];  //< referenced from SubCore
    static SoundCapture::Capture capture;   //< referenced from SubCore
    ...
    if (param.size > 0) {
        ...
        capture.sample  = param.size / (AS_BITLENGTH_16 / 8) / CAP_CHANNELS;
